	...
	$.fn.seatCharts = function (setup) {
		...
		$.each(settings.map, function(row, characters) {
			...
			if (settings.naming.left) {
				/*$row.append(
					$('<div></div>')
						.addClass('seatCharts-cell seatCharts-space dummy')
			...
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				...
				$row.append(character != '_' && character != 'l' && character != 'x' &&character != 'w'?
					...
					})(settings.naming) :(function(settings) {
						if(tempcharacter!=null && tempcharacter!="_" && tempcharacter != 'l' && tempcharacter != 'x')
							{
							return $('<div></div>').addClass(['seatCharts-cell seatCharts-space'].concat(
									settings.seats[tempcharacter].classes, 
									typeof settings.seats[tempcharacter] == "undefined" ? [] : settings.seats[tempcharacter].classes).join(' ')
								)	;
							}
						else if(character=='l')
						{
							return $('<div></div>').addClass('seatCharts-cell  lavatory-space')	;
						}
						else if(character=='x')
						{
							return $('<div></div>').addClass('seatCharts-cell seatCharts-exit')	;
						}
						else if(character=='w')
						{
							return $('<div></div>').addClass('seatCharts-cell seatCharts-wall')	;
						}
						else
						{
							return $('<div></div>').addClass('seatCharts-cell seatCharts-space')	;
						}
